...
WHERE
  ...
  AND paid_tokens_balance = 0  -- Only fix free users, don't touch paid users
  AND id NOT IN (
    -- Don't reduce tokens for users who already received first 101 bonus
...
-- Note: We use >= 0 instead of > 0 because users can deplete their tokens
DO $$
BEGIN
