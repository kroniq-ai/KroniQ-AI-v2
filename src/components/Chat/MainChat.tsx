...
export const MainChat: React.FC = () => {
  ...
  const getBestModelForQuery = (query: string): string => {
    ...
    // Code/programming tasks → use Claude or GPT-4
    ...
    // Math/reasoning tasks → use Claude or GPT-4
    ...
    // Research/factual → use Gemini or GPT
    ...
    // Quick questions → use fast models
  ...
  const handleSendMessage = async (messageText?: string, attachments?: File[], options?: string[]) => {
    ...
    if (isSuperMode && user?.id) {
      console.log('🚀 Super KroniQ mode - checking usage limits...');
      try {
        ...
        if (!usageStatus.allowed) {
          console.log('🚫 Usage limit hit:', usageStatus.upgradeReason);
          ...
          console.log('✅ Super KroniQ usage check passed');
        }
      } catch (error) {
        console.warn('⚠️ Usage check failed (table may not exist):', error);
    ...
    if (musicKeywords.test(textToSend) || musicRequestPattern.test(textToSend)) {
      ...
      try {
        ...
        if (result && result.url) {
          console.log('✅ Music generated:', result.url);
        ...
        for (const file of attachments) {
          if (file.type.startsWith('text/') ||
            ...
            try {
              ...
              fileContentContext += `\n\n--- File: ${file.name} --- \n${truncatedText}\n--- End of File ---\n`;
              console.log(`📄 Read text from ${file.name}: ${text.length} chars`);
            } catch (err) {
    ...
    try {
      ...
      try {
        ...
        if (isSuperMode) {
          ...
          console.log(`🚀 Super KroniQ auto-selected model: ${bestModel}`);
  ...
  const getAIResponse = async (projectId: string, userMessage: string) => {
    ...
    try {
      ...
      if (!isFree) {
        ...
        if (!access.isPremium) {
          console.error(`🚫 Access denied: User ${user.id} tried to use premium model ${selectedModel}`);
        ...
        console.log(`✅ Access granted: User ${user.id} has premium access for ${selectedModel}`);
      } else {
      ...
      try {
        console.log('🔄 Deducting tokens via Firebase:', { user_id: user.id, tokens: tokensToDeduct, model: selectedModel });
