...
const CanvasWrapper: FC<{ children: ReactNode }> = ({ children }) => (
    <Canvas dpr={[1, 1.5]} frameloop="demand" className="relative h-full w-full">
        {children}
    </Canvas>
);
...
function createStackedPlanesBufferGeometry(
    ...
    const indices = new Uint32Array(n * heightSegments * 6);
...
>(({ material, width, count, height }, ref) => {
    ...
    useFrame((_, delta) => {
        ...
        if (m?.material?.uniforms?.time) {
            m.material.uniforms.time.value += 0.1 * delta;
        }
    });
